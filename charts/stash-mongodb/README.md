...
$ helm repo update
$ helm install stash-mongodb-3.4.17-v3 appscode/stash-mongodb -n kube-system --version=3.4.17-v3
```
...
To install the chart with the release name `stash-mongodb-3.4.17-v3`:
...
To uninstall/delete the `stash-mongodb-3.4.17-v3`:

```console
$ helm delete stash-mongodb-3.4.17-v3 -n kube-system
```
...
| image.tag        | Tag of the image that is used to backup/restore MongoDB database. This is usually same as the database version it can backup. | `3.4.17-v3`     |
...
```console
$ helm install stash-mongodb-3.4.17-v3 appscode/stash-mongodb -n kube-system --version=3.4.17-v3 --set image.registry=stashed
```
...
```console
$ helm install stash-mongodb-3.4.17-v3 appscode/stash-mongodb -n kube-system --version=3.4.17-v3 --values values.yaml
```
