...
$ helm repo update
$ helm install stash-mongodb-4.0.3-v4 appscode/stash-mongodb -n kube-system --version=4.0.3-v4
```
...
To install the chart with the release name `stash-mongodb-4.0.3-v4`:
...
To uninstall/delete the `stash-mongodb-4.0.3-v4`:

```console
$ helm delete stash-mongodb-4.0.3-v4 -n kube-system
```
...
| image.tag        | Tag of the image that is used to backup/restore MongoDB database. This is usually same as the database version it can backup. | `4.0.3-v4`      |
...
```console
$ helm install stash-mongodb-4.0.3-v4 appscode/stash-mongodb -n kube-system --version=4.0.3-v4 --set image.registry=stashed
```
...
```console
$ helm install stash-mongodb-4.0.3-v4 appscode/stash-mongodb -n kube-system --version=4.0.3-v4 --values values.yaml
```
