...
Install MongoDB {{ $v.Major }}.{{ $v.Minor }}.{{ $v.Patch }} backup or restore plugin for Stash as below.

```console
helm repo add appscode https://charts.appscode.com/stable/
helm repo update
helm install stash-mongodb-{{ .version }} appscode/stash-mongodb --version={{ .version }} --namespace=kube-system
```
...
Uninstall MongoDB {{ $v.Major }}.{{ $v.Minor }}.{{ $v.Patch }} backup or restore plugin for Stash as below.

```console
helm delete stash-mongodb--{{ .version }} --namespace=kube-system
```
